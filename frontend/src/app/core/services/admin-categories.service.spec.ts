...
const url = `${environment.apiUrl}/api/admin/categories`;
describe('AdminCategoriesService', () => {
  ...
  it('lists with the real filters and Bearer token', () => {
    service.listCategories({ search: ' base ', estado: false, page: 2, pageSize: 10 }).subscribe();
  ...
  it('creates with an optional field omitted', () => {
    service.createCategory({ nombre: 'Base' }).subscribe();
  ...
  it('edits only supplied fields and preserves explicit null', () => {
    service.updateCategory(7, { descripcion: null }).subscribe();
  ...
  it.each([401, 403, 404, 409, 422, 500])('propagates HTTP %s', (status) => {
    const error = vi.fn();
    service.createCategory({ nombre: 'Base' }).subscribe({ error });
    http
      .expectOne(url)
