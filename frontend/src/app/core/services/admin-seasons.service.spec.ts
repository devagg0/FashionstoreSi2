...
const url = `${environment.apiUrl}/api/admin/seasons`;
describe('AdminSeasonsService', () => {
  let service: AdminSeasonsService;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(AdminSeasonsService);
  ...
  it('lists with the real filters and Bearer token', () => {
    service.listSeasons({ search: ' base ', estado: false, page: 2, pageSize: 10 }).subscribe();
  ...
  it('creates with an optional field omitted', () => {
    service.createSeason({ nombre: 'Base' }).subscribe();
  ...
  it('edits only supplied fields and preserves explicit null', () => {
    service.updateSeason(7, { descripcion: null }).subscribe();
  ...
  it.each([401, 403, 404, 409, 422, 500])('propagates HTTP %s', (status) => {
    const error = vi.fn();
    service.createSeason({ nombre: 'Base' }).subscribe({ error });
    http
      .expectOne(url)
