...
const url = `${environment.apiUrl}/api/admin/sizes`;
describe('AdminSizesService', () => {
  let service: AdminSizesService;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(AdminSizesService);
  ...
  it('lists with the real filters and Bearer token', () => {
    service.listSizes({ search: ' base ', estado: false, page: 2, pageSize: 10 }).subscribe();
  ...
  it('creates with an optional field omitted', () => {
    service.createSize({ nombre: 'Base' }).subscribe();
  ...
  it.each([401, 403, 404, 409, 422, 500])('propagates HTTP %s', (status) => {
    const error = vi.fn();
    service.createSize({ nombre: 'Base' }).subscribe({ error });
    http
      .expectOne(url)
