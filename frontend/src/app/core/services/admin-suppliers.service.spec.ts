...
describe('AdminSuppliersService', () => {
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(AdminSuppliersService);
  ...
  it('lists with the real filters and Bearer token', () => {
    service.listSuppliers({ search: ' base ', estado: false, page: 2, pageSize: 10 }).subscribe();
  ...
  it('creates with an optional field omitted', () => {
    service.createSupplier({ nombre: 'Base' }).subscribe();
  ...
  it.each([401, 403, 404, 409, 422, 500])('propagates HTTP %s', (status) => {
    const error = vi.fn();
    service.createSupplier({ nombre: 'Base' }).subscribe({ error });
    http
      .expectOne(url)
