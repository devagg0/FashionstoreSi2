...
const url = `${environment.apiUrl}/api/admin/collections`;
describe('AdminCollectionsService', () => {
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(AdminCollectionsService);
  ...
  it('lists with the real filters and Bearer token', () => {
    service.listCollections({ search: ' base ', estado: false, page: 2, pageSize: 10 }).subscribe();
  ...
  it('creates with an optional field omitted', () => {
    service.createCollection({ nombre: 'Base' }).subscribe();
  ...
  it('edits only supplied fields and preserves explicit null', () => {
    service.updateCollection(7, { descripcion: null }).subscribe();
  ...
  it('reads detail and changes status without deletion', () => {
    service.getCollection(7).subscribe();
  ...
  it.each([401, 403, 404, 409, 422, 500])('propagates HTTP %s', (status) => {
    const error = vi.fn();
    service.createCollection({ nombre: 'Base' }).subscribe({ error });
    http
      .expectOne(url)
